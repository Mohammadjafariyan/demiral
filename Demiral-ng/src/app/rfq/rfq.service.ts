...
import {_baseUrl} from "../app-service";
...
export class RFQsClient {
  ...
  constructor(@Inject(Http) http: Http, @Optional() @Inject(API_BASE_URL) baseUrl?: string) {
    this.http = http;
    this.baseUrl = baseUrl ? baseUrl : _baseUrl;
  }

  search(searchterm: string): Observable<Category[] | null> {
    let url_ = this.baseUrl + "/api/RFQs/Search?";
  ...
  submit(rfq: RFQ): Observable<void> {
    let url_ = this.baseUrl + "/api/RFQs/Submit";
    url_ = url_.replace(/[?&]$/, "");

    const content_ = JSON.stringify(rfq ? rfq.toJSON() : null);
  ...
  protected processSubmit(response: Response): Observable<void> {
    const status = response.status;
  ...
  details(id: number): Observable<Category | null> {
    let url_ = this.baseUrl + "/api/RFQs/detail?";
    if (id === undefined)
  ...
  protected processDetails(response: Response): Observable<Category | null> {
    const status = response.status;
